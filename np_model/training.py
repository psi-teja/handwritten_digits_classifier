...
for j in trange(epochs, desc="Training In Progress", unit="epochs"):
    ...
    while i + batch_size < N:
        ...
        grad_wo = np.dot(hl2_a.transpose(), d_out)   
        ...
        w1 += grad_w1 * (initial_learning_rate)

        bo += grad_bo * (initial_learning_rate)
        b2 += grad_b2 * (initial_learning_rate)
        b1 += grad_b1 * (initial_learning_rate)
...
pkl.dump(weights, open('weights/model_weights.pkl', 'wb'))
